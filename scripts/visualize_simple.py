...
for i, bank in enumerate(df["bank"].unique()):
    ...
    rating_counts.plot(kind="bar", ax=axes[i], color="steelblue")
...
sentiment_by_rating.plot(kind="bar", stacked=True, color=["#2ecc71", "#e74c3c", "#95a5a6"], ax=ax3)
ax3.set_title("Sentiment Distribution by Rating Score", fontsize=14, fontweight="bold")
...
ax4.set_xlabel("Bank", fontsize=12)
ax4.set_ylabel("Average Sentiment Score (-1 to +1)", fontsize=12)
...
print("  - sentiment_by_rating.png")
print("  - avg_sentiment_by_bank.png")
